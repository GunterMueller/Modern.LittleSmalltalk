...
static noreturn genMessage(BOOL X INT X OBJ);
static noreturn expression(NOARGS);
static noreturn parsePrimitive(NOARGS);
static noreturn block(NOARGS);
static noreturn body(NOARGS);
static noreturn assignment(STR);

noreturn setInstanceVariables(aClass)
object aClass;
{	int i, limit;
	object vars;
...
static noreturn genInstruction(high, low)
int high, low;
{
	if (low >= 16) {
...
static noreturn genInteger(val)	/* generate an integer push */
int val;
{
	if (val == -1)
...
static noreturn parsePrimitive()
...
static noreturn genMessage(toSuper, argumentCount, messagesym)
boolean toSuper;
int argumentCount;
object messagesym;
{	boolean sent = false;
...
static noreturn continuation(superReceiver)
boolean superReceiver;
{
...
static noreturn assignment(name)
char *name;
{	int i;
	boolean done;
...
static noreturn messagePattern()
{	object argsym;
